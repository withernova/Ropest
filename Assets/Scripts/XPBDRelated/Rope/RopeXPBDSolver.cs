...
public class RopeXPBDSolver : XPBDSolver, IControllable
{
    ...
    {
        if (PointData.datas[i].interactiveItem is InteractiveSwing)
        {
            var item = PointData.datas[i];
            pointPos[item.index] = Vector3.Lerp(pointPos[item.index], item.interactiveItem.transform.position, 0.2f);
            if ((item.interactiveItem.transform.position - pointPos[item.index]).magnitude < 1e-4)
            {
                pointPos[item.index] = item.interactiveItem.transform.position;
            }
        }
    ...
    public void Swing(InteractiveSwing target)
    {
        ...
        {
            if (PointData.datas[ctrlIndex].interactiveItem is InteractiveGrab) return;
            CancelSwing(ctrlIndex);
            return;
        }
    ...
    public void CancelGrab(int i)
    {
        ((InteractiveGrab)PointData.datas[i].interactiveItem).SetV(vel[i]);
        pointInvMass[i] = PointData.datas[i].Reset();
        gravityFactor -= 0.1f;
        pointInvMass[ctrlIndex] = ctrlMass;
    }

    public void CancelSwing(int i)
    {
        pointInvMass[i] = PointData.datas[i].Reset();
        pointInvMass[ctrlIndex] = ctrlMass;
    }
    ...
    public void Grab(InteractiveGrab target)
    {
        if (PointData.datas[ctrlIndex].isActivating)
        {
            if (PointData.datas[ctrlIndex].interactiveItem is InteractiveSwing) return;
            CancelGrab(ctrlIndex);
            return;
        }
